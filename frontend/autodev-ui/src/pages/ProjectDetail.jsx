...
function SetupInstructions({ projectName }) {
  ...
  useEffect(() => {
    ...
    api.getSetupInstructions(projectName)
        ...
        {instructions.map((step, i) => (
          <div key={i} className="space-y-2">
            ...
            {step.commands?.length > 0 && (
              <div className="ml-7 bg-[#0d0d0d] border border-border rounded-lg overflow-hidden">
                {step.commands.map((cmd, j) => (
                  <div key={j} className="flex items-center gap-2 px-3 py-2 border-b border-border last:border-0 group">
                    <span className="text-success text-xs shrink-0">$</span>
                    <code className="text-xs font-mono text-slate-300 flex-1 select-all">{cmd}</code>
                    <button
                      onClick={() => navigator.clipboard.writeText(cmd)}
                      className="opacity-0 group-hover:opacity-100 text-slate-600 hover:text-slate-300 text-xs transition-all"
                    >
                      copy
                    </button>
                  </div>
...
export default function ProjectDetail() {
  ...
  return (
    <div className="p-6 space-y-5 max-w-5xl">
      ...
      <SetupInstructions projectName={id} />
